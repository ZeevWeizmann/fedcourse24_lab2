...
class FedProxLearner(Learner):
    def fit_epoch(self, loader, global_model=None, mu=0.0):
        self.model.train()
        for x, y in loader:
            x = x.to(self.device).type(torch.float32)
            y = y.to(self.device).type(torch.long)
            ...
            if global_model is not None and mu > 0:
                prox_term = 0.0
                with torch.no_grad():
                    global_params = [p.detach() for p in global_model.parameters()]
    ...
    def fit_epochs(self, loader, n_epochs, global_model=None, mu=0.0):
        for _ in range(n_epochs):
            self.fit_epoch(loader, global_model=global_model, mu=mu)
