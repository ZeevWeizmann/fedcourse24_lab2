...
*Figure 2: Effect of local epochs on the test accuracy under non-IID data distribution.*
...
### Objective
Implement two client sampling strategies from the research paper *"On the Convergence of FedAvg on Non-IID Data".*
...
*Figure 3: Test accuracy across communication rounds for different sampling rates. Plot clearly indicates that increasing the sampling rate improves model accuracy and stability, but also increases the overall computation time.*
...
*Figure 4: Test accuracy over 100 rounds for sampling with replacement. As expected, higher sampling rates lead to more stable convergence and higher final accuracy, since more clients participate in each round, reducing variance in the aggregated model.*
...
### Background
Federated learning often suffers from *client drift* when local datasets are heterogeneous (non-iid).
**FedProx** was proposed to address this issue by modifying the local optimization objective with a *proximal term* that penalizes the deviation of local model parameters from the global model.
...
*Figure 5: Test accuracy of FedAvg vs. FedProx for various numbers of local steps.*
...
*Figure 6: Test metric across communication rounds for SCAFFOLD (TensorBoard visualization).*
